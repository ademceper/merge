...
public class ExportReportQueryHandler : IRequestHandler<ExportReportQuery, byte[]?>
{
    ...
    public async Task<byte[]?> Handle(ExportReportQuery request, CancellationToken cancellationToken)
    {
        ...
        if (report == null)
        {
            _logger.LogWarning("Report not found for export. ReportId: {ReportId}", request.Id);
        ...
        _logger.LogInformation("Report exported successfully. ReportId: {ReportId}, DataSize: {DataSize} bytes", request.Id, data.Length);
