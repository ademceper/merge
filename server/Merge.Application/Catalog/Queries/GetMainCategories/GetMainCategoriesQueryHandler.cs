...
public class GetMainCategoriesQueryHandler : IRequestHandler<GetMainCategoriesQuery, PagedResult<CategoryDto>>
{
    ...
    {
        _logger.LogInformation("Retrieving main categories. Page: {Page}, PageSize: {PageSize}",
        ...
        var cachedResult = await _cache.GetOrCreateAsync(
            ...
            async () =>
            {
                _logger.LogInformation("Cache miss for main categories (paged). Fetching from database.");
