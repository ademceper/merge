...
public class PublishCMSPageCommandHandler : IRequestHandler<PublishCMSPageCommand, bool>
{
    private readonly Merge.Application.Interfaces.IRepository<CMSPage> _cmsPageRepository;
    private readonly IDbContext _context;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ICacheService _cache;
    private readonly ILogger<PublishCMSPageCommandHandler> _logger;
    ...
    public PublishCMSPageCommandHandler(
    ...
    {
        _logger.LogInformation("Publishing CMS page. PageId: {PageId}", request.Id);
        ...
        try
        {
            ...
            // ✅ BOLUM 3.2: IDOR Korumasi - Manager sadece kendi sayfalarını yayınlayabilmeli (Admin hariç)
            if (request.PerformedBy.HasValue && page.AuthorId.HasValue && page.AuthorId.Value != request.PerformedBy.Value)
            {
                _logger.LogWarning("Unauthorized attempt to publish CMS page {PageId} by user {UserId}. Page belongs to {AuthorId}",
            ...
            _logger.LogInformation("CMS page published successfully. PageId: {PageId}", request.Id);
        ...
        catch (DbUpdateConcurrencyException ex)
        {
            ...
            _logger.LogError(ex, "Concurrency conflict while publishing CMS page. PageId: {PageId}",
                request.Id);
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Error publishing CMS page. PageId: {PageId}", request.Id);
            await _unitOfWork.RollbackTransactionAsync(cancellationToken);
            throw;
        }
    }
